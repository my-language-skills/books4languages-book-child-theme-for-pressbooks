...
$key_h5p_form_id_field = get_post_meta( get_the_ID(), 'efp_h5p_form_id_field', true );
$key_h5p_use_id_field = get_post_meta( get_the_ID(), 'efp_h5p_use_id_field', true );
  ...
  if ( is_singular('chapter')) {
    ...
    <?php
      if ( !empty($key_h5p_form_id_field)) { ?>
      <p>External link to <a target="_blank" rel="noopener noreferrer"  onclick="gtag('event', 'visit_exercise', {'event_category': 'resources', 'event_label': 'worksheet'});" href="https://worksheet.books4languages.com/english/wp-admin/admin-ajax.php?action=h5p_embed&id=<?php echo $key_h5p_form_id_field?>"><?php _e( ' Form exercises ' . the_title() . ' (' . $key_h5p_form_id_field . ')' , 'pressbooks-book' ); ?></a>.</p> <?php
      }
      if ( !empty($key_h5p_use_id_field)) { ?>
      <p>External link to <a target="_blank" rel="noopener noreferrer"  onclick="gtag('event', 'visit_exercise', {'event_category': 'resources', 'event_label': 'worksheet'});" href="https://worksheet.books4languages.com/english/wp-admin/admin-ajax.php?action=h5p_embed&id=<?php echo $key_h5p_use_id_field?>"><?php _e( ' Use exercises ' . the_title() . ' (' . $key_h5p_use_id_field . ')' , 'pressbooks-book' ); ?></a>.</p> <?php
      } ?>
      <br>  <?php
      ...
      <input class="summaryblock1" type="submit" name="button1" value="Show exercises"/>
      <input class="summaryblock2" type="submit" name="button2" value="Hide exercises"/>
      </form>
      <br>
      </div>
      </div>
...
$key_h5p_use_id_field = get_post_meta( get_the_ID(), 'efp_h5p_use_id_field', true );
   ...
   * Exercises section
   * @internal Spanish
   * @link https://worksheet.books4languages.com/spanish/
   * @link https://books4languages.com/contribute/
   *
   * @since 1.x
   *
...
function b4l_h5pexercises_spanish() {
$key_h5p_id_field = get_post_meta( get_the_ID(), 'efp_h5p_id_field', true );
$key_h5p_form_id_field = get_post_meta( get_the_ID(), 'efp_h5p_form_id_field', true );
$key_h5p_use_id_field = get_post_meta( get_the_ID(), 'efp_h5p_use_id_field', true );
  ...
  if ( is_singular('chapter')) {
    ...
    <div class="textbox__content">
    <p>External link to <a target="_blank" rel="noopener noreferrer"  onclick="gtag('event', 'visit_exercise', {'event_category': 'resources', 'event_label': 'worksheet'});" href="https://worksheet.books4languages.com/spanish/wp-admin/admin-ajax.php?action=h5p_embed&id=<?php echo $key_h5p_id_field?>"><?php _e( ' exercises ' . the_title() . ' (' . $key_h5p_id_field . ')' , 'pressbooks-book' ); ?></a>.</p>
    <?php
      if ( !empty($key_h5p_form_id_field)) { ?>
      <p>External link to <a target="_blank" rel="noopener noreferrer"  onclick="gtag('event', 'visit_exercise', {'event_category': 'resources', 'event_label': 'worksheet'});" href="https://worksheet.books4languages.com/spanish/wp-admin/admin-ajax.php?action=h5p_embed&id=<?php echo $key_h5p_form_id_field?>"><?php _e( ' Form exercises ' . the_title() . ' (' . $key_h5p_form_id_field . ')' , 'pressbooks-book' ); ?></a>.</p> <?php
      }
      ...
      if ( is_user_logged_in() ) {  // Check if visitor
      ?>
      <iframe src="https://worksheet.books4languages.com/spanish/wp-admin/admin-ajax.php?action=h5p_embed&id=<?php echo $key_h5p_id_field?>" frameborder="0" allowfullscreen="allowfullscreen"></iframe>
      <script src="https://worksheet.books4languages.com/content/wp-content/plugins/h5p/h5p-php-library/js/h5p-resizer.js" charset="UTF-8"></script>
      <br> <?php
      }
      if ( !is_user_logged_in()) {  // Check if student.
  ...
  add_shortcode('h5pexercises_spanish','b4l_h5pexercises_spanish'); // Code to shorcode.
